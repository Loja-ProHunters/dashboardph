...
function drawProductsTable(doc, produtos) {
  ...
  produtos.forEach(p => {
    // altura dinâmica pra complemento longo
    const compLines = Math.ceil((doc.font.widthOfTextAtSize(String(p.complemento || ''), 7.5)) / (cols[1].w - 6)) || 1;
    const rowH = Math.max(26, 10 + compLines * 9);
    x = M;
    cols.forEach(c => {
      doc.rect(x, doc.y, c.w, rowH);
      if (c.key === 'complemento') {
        doc.wrapText(p.complemento, x + 3, doc.y - 10, c.w - 6, { size: 7.5, lineGap: 9 });
      } else {
...
async function drawSeloBarcodeValidade(doc, { seloNumero, assinanteNome, assinanteCargo, dataStr, validadeStr }) {
  ...
  doc.text('Selo Número: ' + seloNumero, M + 5, doc.y - 36, { size: 9, bold: true });
  doc.rect(M + halfW + 8, doc.y, halfW, h1);
  doc.text(assinanteNome, M + halfW + 13, doc.y - 20, { size: 9, bold: true });
  doc.text(assinanteCargo, M + halfW + 13, doc.y - 33, { size: 8.5 });
  doc.y -= h1;
  ...
  doc.text('Guia de Tráfego Válida até: ' + validadeStr, M + 5, doc.y - 17, { size: 9, bold: true });
  doc.y -= h3;
}
...
async function gerarGTPagina(pdfCommon, data, destinatarios) {
  ...
  doc.field(M + w1, doc.y, w2, hRow, 'SFPC', destinatarios[0].sfpc, { bold: true });
  ...
  doc.text('Emitido por: ' + companies.representante.nome.toUpperCase() + ' - ' + data.dataEnvio, M, doc.y - 4, { size: 7.5 });
}
...
async function gerarGT(data) {
  const crestBytes = loadAsset('exercito_brasao.jpg');
