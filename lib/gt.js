...
async function drawHeader(doc, crestBytes) {
  ...
  // brasão à esquerda, textos centralizados na página
...
// Linha superior: NÚMERO DA GUIA / SFPC / Folha ; NOTA FISCAL / DATA ; NÚMERO DE VOLUMES
// Layout de "label : valor" na mesma linha, dentro de caixas — igual ao modelo oficial.
function drawTopoInfo(doc, data) {
  const boxX = M + 70;          // as caixas do topo começam recuadas (como no modelo)
  const boxW = CW - 140;        // e não ocupam a largura toda
  const rowH = 18;

  // Linha 1: NÚMERO DA GUIA : xxx | SFPC/05 | Folha : 1 de 1
  ...
  // Linha 2: NOTA FISCAL Nº : xxx | DATA: xxx
  doc.rect(boxX, doc.y, boxW, rowH);
  ...
  // Linha 3: NÚMERO DE VOLUMES: x  (centralizado)
  doc.rect(boxX, doc.y, boxW, rowH);
...
// Bloco de parte (empresa/transportador/destinatário) no layout do modelo oficial:
// título à esquerda, nome centralizado, CNPJ à direita; endereço na linha de baixo,
// telefone à direita; separadores horizontais entre os blocos.
function drawParteOficial(doc, { titulo, nome, doc: docNum, endereco, telefone, sfpc, extraNome }) {
  ...
  const nomeW = doc.font.widthOfTextAtSize(nomeFull, 9);
  const nameSize = nomeFull.length > 50 ? 7 : 9;
  const nomeW2 = doc.font.widthOfTextAtSize(nomeFull, nameSize);
  doc.text(nomeFull, cx - nomeW2 / 2, doc.y - 10, { size: nameSize, bold: false });
  ...
  doc.y -= 4;
  doc.line(M, doc.y, M + CW, doc.y); // separador
  doc.y -= 8;
}
...
function drawProductsTable(doc, produtos) {
  ...
  produtos.forEach(p => {
    ...
    cols.forEach(c => {
      ...
      if (c.wrap) {
        // texto que pode quebrar: centraliza verticalmente estimando o nº de linhas
...
async function drawSeloBarcodeValidade(doc, { seloNumero, assinanteNome, assinanteCargo, dataStr, validadeStr }) {
  ...
  // Caixa grande esquerda (selo) e direita (assinatura) — 2 linhas de altura
  const h1 = 100; // parte de cima: "selo de autenticidade / obrigatorio" | nome+cargo
  const h2 = 72; // parte de baixo: codigo de barras | data
  ...
  // ── DIREITA baixo: "Blumenau (SC), data." ──
  ...
  // ── Validade (largura toda, centralizado, negrito) ──
  const h3 = 22;
...
async function gerarGTPagina(pdfCommon, data, destinatarios) {
  ...
  drawParteOficial(doc, {
    ...
    nome: emp.codigo + ' – ' + emp.nome.split(' ').slice(0, 2).join(' '),
    extraNome: emp.nome.split(' ').slice(2).join(' '),
    doc: emp.cnpj,
