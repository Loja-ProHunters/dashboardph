...
// ── Mapeia vendedor Bling → owner CRM ────────────────────────────
// Casa por email do vendedor Bling com email do users.json do CRM.
// Se não achar, retorna null (Order.vendedor_id fica vazio, gerência resolve).
function mapearVendedor(pedidoDetalhe, usersJson) {
  if (!pedidoDetalhe || !pedidoDetalhe.vendedor) return null;
  const v = pedidoDetalhe.vendedor;
  const emailBling = String(v.email || v.contato?.email || '').toLowerCase().trim();
  const nomeBling  = String(v.nome || v.contato?.nome || '').toLowerCase().trim();
  if (!usersJson) return null;
  // Tenta email primeiro (mais confiável)
  if (emailBling) {
    for (const [login, u] of Object.entries(usersJson)) {
      if (u && u.email && String(u.email).toLowerCase() === emailBling) return login;
    }
  }
  // Fallback: nome contém o login (mathias, dickmann, boschetto)
  if (nomeBling) {
    for (const login of Object.keys(usersJson)) {
      if (nomeBling.includes(login.toLowerCase())) return login;
    }
  }
  return null;
}
...
function processarPedido(detalhe, snapshot, ctx = {}) {
  const avisos = [];
  ...
  const vendedorLogin = mapearVendedor(detalhe, usersJson);
