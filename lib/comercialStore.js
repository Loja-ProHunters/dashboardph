...
// Soma no bucket do SITE. Não é vendedor: não entra no rank/score/comissão.
// Só compõe o Total Geral do mês (para gerência acompanhar quanto o site
// vendeu). Usa o mesmo storage do resto do dashboard comercial.
async function registrarVendaSite(valor) {
  const data = await getComercialData();
  data.siteFat = (data.siteFat || 0) + valor;
  await saveComercialData(data);
  return data;
}
