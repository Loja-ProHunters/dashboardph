...
const CAMPOS_CONDICIONAIS = {
  cnpj:  ['anexo_p_link', 'gru_link'],
  aereo: ['comprovante_pix_link', 'peso_kg', 'dimensoes_cm', 'minuta_numero', 'aeroporto_iata'],
};
...
const ALLOWED_PATCH_KEYS = [
  ...
  // condicionais AÉREO
  'comprovante_pix_link', 'pix_baixado', 'peso_kg', 'dimensoes_cm', 'minuta_numero', 'aeroporto_iata',
  // meta
...
function estadoPasso(env, passo) {
  ...
  switch (passo.id) {
    ...
    case 'liberacao': {
      ...
      if (_isCNPJ(c) && (!c.anexo_p_link || !c.gru_link)) return 'travado';
...
function _condicionaisPendentes(env) {
  const p = [];
  if (_isCNPJ(env)) {
    if (!env.anexo_p_link) p.push('anexo_p_link');
    if (!env.gru_link) p.push('gru_link');
  }
  if (_isAereo(env)) {
...
// Aplica patch: só campos da whitelist, carimba autor/hora quando aplicável.
function aplicarPatch(envAtual, patch, actor) {
  ...
  for (const [k, v] of Object.entries(patch || {})) {
    ...
    out[k] = v;
    eventos.push({ ts: now, por: actor || null, acao: 'campo_alterado', campo: k, novo: v });
