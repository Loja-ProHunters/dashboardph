...
function canAccessCRM(sess) {
  // Em v1: gerência e vendas. Auxiliar fica fora do CRM geral, mas TEM acesso
  // específico à Operação Controlado (ver canAccessControlado abaixo).
  const r = role(sess);
  return r === 'admin' || r === 'vendas';
}

// A auxiliar precisa operar a fila de envios controlados, romaneios e cotação de frete.
// Esse acesso é liberado explicitamente aqui pra não misturar com o CRM inteiro.
function canAccessControlado(sess) {
  const r = role(sess);
  return r === 'admin' || r === 'vendas' || r === 'auxiliar';
}
...
module.exports = {
  ...
  role, canAccessCRM, canAccessControlado, canSeeAll, canReadDoc, canWriteDoc, scopeFilter, enforceOwner,
  isEnum,
};
