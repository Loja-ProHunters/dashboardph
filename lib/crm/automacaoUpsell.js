...
// senao rodizio round-robin pros vendedores ativos (baseado em hash do
// accountId, ficando deterministico — mesmo cliente cai sempre no mesmo
// vendedor de rodizio, evita bagunca de "quem falou com quem").
//
// "gerencia" e strings vazias sao tratados como "sem vendedor" e caem no
// rodizio. Isso porque a importacao do Bling carimba owner_id='gerencia'
// quando nao acha o vendedor original — tratar isso como dono real deixaria
// as tarefas empilhadas pra gerencia em vez de circular pros vendedores.
function _resolverVendedor(account, meusPeds) {
  if (meusPeds && meusPeds.length && meusPeds[0].vendedor_id) {
    ...
    if (_isVendedorReal(v)) return v;
  }
  if (account && account.owner_id) {
    const v = _normLogin(account.owner_id);
    if (_isVendedorReal(v)) return v;
  }
  return _rodizio(account && account.id);
}

// So considera vendedor "real" quem esta na lista ativa. Isso impede que
// vendedores desligados (nomes que apareceram em pedidos antigos mas nao
// estao mais na empresa) recebam tarefas — cai no rodizio pros atuais.
// Se voce quer aceitar temporariamente um vendedor fora da lista, adicione
// ele em VENDEDORES_ATIVOS no topo do arquivo.
function _isVendedorReal(login) {
  if (!login) return false;
  if (login === 'gerencia' || login === 'admin' || login === 'auxiliar') return false;
